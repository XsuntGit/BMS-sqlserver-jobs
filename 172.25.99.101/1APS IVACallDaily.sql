...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'IVACallDaily', 
		@step_id=1, 
		...
		@command=N'exec [dbo].[sp_ALL_IVA]', 
		@database_name=N'BMSRData_CSCAN', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'IVA_ALL', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=64, 
		@freq_subday_type=1, 
		...
		@active_start_time=105500, 
		@active_end_time=235959, 
		@schedule_uid=N'3b6a6648-2f81-4b0b-a4d3-0c87ac57082c'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
