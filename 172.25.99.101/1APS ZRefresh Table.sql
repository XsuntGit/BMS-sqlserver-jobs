/*
	Created by XSUNT\svc-jenkins using dbatools Export-DbaScript for objects on 172.25.99.101 at 08/06/2023 21:22:24
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'1APS_ZRefresh_Table', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'refresh tables logs step1', 
		@step_id=1, 
...
print @maxdate
----if OBJECT_ID ( N''dashONC30_LogVisits_main'') is not null
----Drop Table dashONC30_LogVisits_main
----Select * into  dashONC30_LogVisits_main from [onelook-sql].bmsonc_867.dbo.dashONC30_LogVisits
...
when FORMAT(DateEntered ,''dddd'') =''Saturday'' then 7 end as WkNum,Wk_Str   from [onelook-sql].bmsonc_867.dbo.dashONC30_LogVisits t1 with(nolock)
 left join bmsonc3..tblweekdate t3 with(nolock) on cast(t3.Wdate as date)  = cast(dateentered as date)
...
----Select * into  tblreproster from [onelook-sql].Bmsonelook.dbo.tblreproster
...
,cast(a.Wk as date) Wk
  ...
  where empemail like ''%@bms.com''   
  and SFA in (
 Select distinct sfcd from bmsonc301..outputgeo) 
...
from
    (
        Select distinct top 100000
            Wk
        from
           Test_Prateek..dashONC30_LogDBMTBM_proc order by 1 
    )
    c
...
from(
Select * from (
 Select DataType,InfoGrp,Info,Wk,Count(OncOriginal)Ct   from Test_Prateek..dashONC30_LogDBMTBM_proc
...
,cast(Information as varchar) Info
  ...
  inner join   tblRepRoster b on a.username = b.emplogin
  ---left join bmsonc3..tblweekdate t3 on cast(t3.Wdate as date)  = cast(dateentered as date)
  left join BMSONC3..OutputGeo t4 on t4.geo =left(cast(UserGeo as varchar),2)+''000000''
  where empemail like ''%@bms.com'' 
...
----order by infogrp,wk 
       ...
       EXEC msdb.dbo.sp_send_dbmail
           ...
           , @subject      = ''Log Table Refreshed :) ''
           , @body         = ''Log Table Refreshed''
           , @body_format  = ''HTML''
	', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'recurring refresh', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_start_date=20210412, 
		@active_end_date=99991231, 
		@active_start_time=193000, 
		@active_end_time=235959, 
		@schedule_uid=N'0c269bba-e8b1-4bb9-b5ba-51e38e6de87e'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
