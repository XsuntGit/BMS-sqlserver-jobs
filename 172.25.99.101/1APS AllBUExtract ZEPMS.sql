...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'ZEPMS EXTRACT RUN STEP', 
		@step_id=1, 
...
set @str1 = cast(FORMAT(GETDATE() , ''yyyyMMdd_HHmmss'') as varchar)
	...
	set @Sql = ''SQLCMD -S ONELOOK-DB-1 -i "W:\work\scripts\BMS\OneLook\ONC\BMS Oncology\AutomationWeeklyExtractScripts\DataExtractForALLBU\ZEPMS\Job.sql" -o "W:\work\scripts\BMS\OneLook\ONC\BMS Oncology\AutomationWeeklyExtractScripts\DataExtractForALLBU\ZEPMS\Logs\ZEPMS_Logs_''+@str1+''.txt"''
